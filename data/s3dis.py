...
class S3DIS(Dataset):
    def __init__(self, cfg, file_names, aug_options, test=False, unlabeled=False):
        ...
        self.subsample_voxel_size = cfg.subsample_voxel_size
    ...
    def __getitem__(self, id):
        ...
        # subsample
        if self.subsample:
            xyz_origin_t = torch.from_numpy(((xyz_origin - xyz_origin.min(0)) / self.subsample_voxel_size)).long()
            xyz_origin_t = torch.cat([torch.LongTensor(xyz_origin_t.shape[0], 1).fill_(0), xyz_origin_t], 1)
            voxel_xyz, p2v_map, v2p_map = voxelization_idx(xyz_origin_t, 1, 4)
            subsample_idx = v2p_map[:, 1]

            xyz_origin = xyz_origin[subsample_idx]
            rgb = rgb[subsample_idx]
            if not self.test:
                label = label[subsample_idx]

        if self.unlabeled:
            # get overlapped crops
            while (True):
                idx1, idx2, idxo, idxo1, idxo2 = get_overlap_xy_crops(xyz_origin, self.crop_size, self.crop_max_iters)
                if idxo.__len__() > 1000:
                    break
...
class MyDataset:
    ...
    def trainLoader(self):
        self.train_file_names = sorted(glob.glob(os.path.join(self.data_root, self.dataset,
                                                              's3dis', self.train_area + '*.pth')))
        ...
        l_train_set = S3DIS(
            ...
            # [jit, flip, rot, elastic, crop, rgb_aug, subsample]
            unlabeled=False
        )
    ...
    def valLoader(self):
        self.val_file_names = sorted(glob.glob(os.path.join(self.data_root, self.dataset,
                                                            's3dis', self.test_area + '*.pth')))
        val_set = S3DIS(
    ...
    def testLoader(self):
        self.test_file_names = sorted(glob.glob(os.path.join(self.data_root, self.dataset,
                                                             's3dis', self.test_area + '*.pth')))
        self.test_set = S3DIS(
